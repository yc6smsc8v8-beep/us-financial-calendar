...
# --- Config ---
FMP_API_KEY = os.getenv("FMP_API_KEY", "jGOOgPs7zXMPkcultsP6M01SrYIm15nV")  # override in Render env if desired
TZ = pytz.timezone("America/New_York")
UTC = pytz.utc
LOOKAHEAD_DAYS = int(os.getenv("LOOKAHEAD_DAYS", "365"))  # default 12 months
...
def fetch_earnings_calendar(from_date: str, to_date: str):
    try:
        r = _get(f"{FMP_BASE}/earning_calendar", params={"from": from_date, "to": to_date, "apikey": FMP_API_KEY})
...
def fetch_economic_calendar(from_date: str, to_date: str):
    try:
        r = _get(f"{FMP_BASE}/economic_calendar", params={"from": from_date, "to": to_date, "apikey": FMP_API_KEY})
        return r.json() if r else []
    except Exception:
        return []

# --- Build ICS (Outlook-friendly: UTC, DTEND) ---
...
def debug():
    try:
        ...
        return jsonify({
            ...
            "first_event": events[0]["summary"] if events else None
        })
...
def index():
    return (
        ...
        '<p>Health check: <a href="/health">/health</a> | '
